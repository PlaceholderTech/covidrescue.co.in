...
async function sendNotification({ user, centers }){
    ...
    mailer.sendEmail(user.email, 'VACCINATION SLOTS AVAILABLE', mailBody, (err, result) => {
        if(err) {
            console.error({err});
        }
    })
...
export default async (req, res) => { 
        ...
        .then(response => {
            ...
            if ( response?.data?.centers.length !== 0 ) {
                response.data.centers.forEach((center) => { 
                    ...
                    if (!distrcitsCentersData[uniqueDistrictIds[index]][center.pincode]) {
                        distrcitsCentersData[uniqueDistrictIds[index]][center.pincode] = {} 
                        distrcitsCentersData[uniqueDistrictIds[index]][center.pincode][center.center_id] = []
                        distrcitsCentersData[uniqueDistrictIds[index]][center.pincode][center.center_id].push(center)
                    } else {
                        if (!distrcitsCentersData[uniqueDistrictIds[index]][center.pincode][center.center_id])
                            distrcitsCentersData[uniqueDistrictIds[index]][center.pincode][center.center_id] = []
    ...
    usersData.forEach((user) => {
        ...
        if (user.pincode && user.pincode.length === 6)
            ...
            Object.keys(distrcitsCentersData[user.district_id]).forEach((pincode) => {
                covidCenters = {...covidCenters, ...distrcitsCentersData[user.district_id][pincode]}
            })
